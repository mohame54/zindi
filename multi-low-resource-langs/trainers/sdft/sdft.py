...
class DistilTrainer(BaseTrainer):
    ...
    def _generate_single_turn(self, prompts: list[str], images: Optional[list]):
        ...
        prompts_text = [self._apply_prompt_template(prompt) for prompt in prompts]
    ...
    def _apply_prompt_template(self, prompt):
        if is_conversational({"prompt": prompt}) and self.chat_template_kwargs:
            template_processor = self.processing_class
            if not hasattr(template_processor, "apply_chat_template") and hasattr(template_processor, "tokenizer"):
                template_processor = template_processor.tokenizer
        ...
        return maybe_apply_chat_template({"prompt": prompt}, self.processing_class)["prompt"]
    ...
    def _generate_and_score_completions(
        ...
        teacher_prompts_text = [self._apply_prompt_template(prompt) for prompt in teacher_prompts]
